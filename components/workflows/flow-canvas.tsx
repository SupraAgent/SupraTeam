...
function FlowCanvasInner({ initialNodes, initialEdges, onSave, saving }: FlowCanvasProps) {
  ...
  const onConnect: OnConnect = React.useCallback(
    (connection: Connection) => {
      // Validate: trigger nodes only have source, not target
      const sourceNode = nodes.find((n) => n.id === connection.source);
      const targetNode = nodes.find((n) => n.id === connection.target);
      if (!sourceNode || !targetNode) return;

      // Don't allow connecting to trigger nodes
  ...
  const onDrop = React.useCallback(
    (event: React.DragEvent) => {
      ...
      if (nodeType === "trigger") {
        ...
        data = { nodeType: "condition", label, config: { field: "board_type", operator: "equals", value: "", ...defaultConfig } };
      } else {
