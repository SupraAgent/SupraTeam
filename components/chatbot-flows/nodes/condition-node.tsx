...
export function ChatbotConditionNode({ data, selected }: NodeProps) {
  const nodeData = data as unknown as ConditionNodeData;
  const cfg = nodeData.config;
  const summary = cfg.field
    ? `${cfg.conditionType}: ${cfg.field} ${cfg.operator} ${cfg.value || "?"}`
    : "Configure condition...";
    ...
    >
      ...
      <div className="flex items-center gap-2.5">
        ...
        <div className="min-w-0">
          ...
          <p className="text-[10px] text-yellow-400/70">Branch logic</p>
        </div>
      </div>
