...
import { useShell, type ViewDensity } from "@/app/_components/shell/shell-context";
...
export default function SettingsPage() {
  const { user } = useAuth();
  const { viewDensity, setViewDensity } = useShell();
  ...
  return (
    <div className="space-y-8">
      ...
      {/* View Preferences */}
      <section className="space-y-4">
        <h2 className="text-base font-medium text-foreground">View Preferences</h2>

        <div className="rounded-2xl border border-white/10 bg-white/[0.035] p-5 space-y-4">
          <div>
            <label className="block text-xs text-muted-foreground mb-2">Information Density</label>
              ...
              {(["compact", "comfortable", "spacious"] as ViewDensity[]).map((d) => (
                ...
                >
                  <div className="space-y-1">
                    {d === "compact" && (
                      <div className="flex flex-col gap-0.5 mx-auto w-12">
                        <div className="h-1 rounded-full bg-current opacity-40" />
                        <div className="h-1 rounded-full bg-current opacity-30" />
                        <div className="h-1 rounded-full bg-current opacity-20" />
                      </div>
                    )}
                    {d === "comfortable" && (
                      <div className="flex flex-col gap-1 mx-auto w-12">
                        <div className="h-1.5 rounded-full bg-current opacity-40" />
                        <div className="h-1.5 rounded-full bg-current opacity-30" />
                        <div className="h-1.5 rounded-full bg-current opacity-20" />
                      </div>
                    )}
                    {d === "spacious" && (
                      <div className="flex flex-col gap-1.5 mx-auto w-12">
                        <div className="h-2 rounded-full bg-current opacity-40" />
                        <div className="h-2 rounded-full bg-current opacity-30" />
                        <div className="h-2 rounded-full bg-current opacity-20" />
                      </div>
                    )}
                    <p className="text-[10px] font-medium capitalize">{d}</p>
                  </div>
                </button>
              ))}
            </div>
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-[10px] text-muted-foreground">Press ? to view all shortcuts</p>
            </div>
            <kbd className="rounded border border-white/10 bg-white/5 px-2 py-1 text-xs text-muted-foreground">?</kbd>
          </div>
        </div>
      </section>
