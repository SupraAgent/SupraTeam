...
/** GET /api/loop/alerts?workflow_id=X — list alert rules for Loop Builder workflows */
export async function GET(request: Request) {
  ...
  let query = supabase
    ...
    .select("id, workflow_id, alert_type, channel, config, is_active, created_at, crm_workflows!workflow_id(name, builder_type)")
  ...
  const loopAlerts = (data ?? []).filter((a: Record<string, unknown>) => {
    const wf = a.crm_workflows as { builder_type?: string } | null;
    return wf?.builder_type === "loop";
  });

  return NextResponse.json({ alerts: loopAlerts });
}

/** POST /api/loop/alerts — create an alert rule for a Loop Builder workflow */
export async function POST(request: Request) {
  ...
  // Verify workflow exists and is a Loop Builder workflow
  const { data: wf } = await supabase
    .from("crm_workflows")
    .select("id, builder_type")
    .eq("id", workflow_id)
    .single();
  ...
  if ((wf as Record<string, unknown>).builder_type !== "loop") {
    return NextResponse.json({ error: "Workflow is not a Loop Builder workflow" }, { status: 400 });
  }
...
/** PATCH /api/loop/alerts?id=X — update an alert rule */
...
/** DELETE /api/loop/alerts?id=X — delete an alert rule */
