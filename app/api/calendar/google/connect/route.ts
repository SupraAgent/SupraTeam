...
export function getCalendarOAuth2Client() {
  ...
  if (!appUrl) throw new Error("NEXT_PUBLIC_APP_URL is required for Calendar OAuth");
...
export function verifyCalendarState(state: string): Record<string, unknown> | null {
  try {
    const key = process.env.TOKEN_ENCRYPTION_KEY;
    if (!key) return null;
...
/** POST: Initiate Google Calendar OAuth flow — returns the authorization URL */
export async function POST() {
  const auth = await requireAuth();
  if ("error" in auth) return auth.error;

  const rl = rateLimit(`cal-oauth-init:${auth.user.id}`, { max: 5, windowSec: 60 });
  if (rl) return rl;
  ...
  const state = signState({ uid: auth.user.id, ts: Date.now(), nonce, type: "calendar" });
  ...
  return NextResponse.json({ url, source: "google_calendar" });
}
