...
import { TOP_ITEMS, NAV_SECTIONS, BOTTOM_ITEMS, ADMIN_ITEM, type NavItem, type NavSection } from "./nav-config";
    ...
    >
      ...
      <nav className="flex-1 px-2 py-3 overflow-y-auto thin-scroll flex flex-col">
        ...
        <div className="mt-3 pt-3 border-t border-white/5 space-y-0.5">
          ...
          {crmRole === "admin_lead" && (
            <NavLink item={ADMIN_ITEM} active={isActive("/admin")} collapsed={sidebarCollapsed} />
          )}
        </div>
      </nav>
      ...
      <div className="border-t border-white/10 p-3">
        {user ? (
          ...
          <Link
            ...
            aria-label={sidebarCollapsed ? "Sign in with Telegram" : undefined}
          >
...
const NavLink = React.memo(function NavLink({ item, active, collapsed }: { item: NavItem; active: boolean; collapsed: boolean }) {
...
// Telegram icon not in lucide-react — keep as inline SVG
